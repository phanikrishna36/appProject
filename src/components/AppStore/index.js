...
class AppStore extends Component {
  state = {
    activeId: 'SOCIAL',
    appsList2: appsList.filter(j => j.category === 'SOCIAL'),
    searchInput: '',
  }

  onBtnClick = tabId => {
    this.setState({
      appsList2: appsList.filter(j => j.category === tabId),
      activeId: tabId,
    })
  }
  ...
  render() {
    const {appsList2, searchInput, activeId} = this.state
    const filteredList = appsList2.filter(i =>
    ...
    return (
      <div className="bg-container">
        ...
        <ul className="buttonStyle" type="none">
          {tabsList.map(i => (
            <TabItem
              key={i.tabId}
              tabDetails={i}
        ...
        <ul className="app" type="none">
          {filteredList.map(i => (
            <AppItem key={i.appId} itemDetails={i} />
          ))}
        </ul>
      </div>
    )
